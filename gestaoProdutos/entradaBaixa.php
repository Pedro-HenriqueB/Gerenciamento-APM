...
@keyframes spin {
  ...
  100% { transform: rotate(360deg); }
	...
	while($indice2 < ($i-1)){
		if(isset($_POST[$uniformes[$indice2][0]])){
			$novoValor = (int)$qtdUniformes[$indice2][0];
			$novoValor += (int)$_POST[$uniformes[$indice2][0]];
			...
			$qtdRegistro = $_POST[$uniformes[$indice2][0]];
			if($qtdRegistro > 0) {
			$novoValor = (float)$qtdRegistro * (float)$valores[$indice2][0];
			$sql = ("INSERT INTO registroUniformes VALUE(null, ?, ?, ?, ?, null)");
			$stmt = $connectar->prepare($sql);
			$stmt->execute([$uniformes[$indice2][0], $qtdRegistro, ($novoValor*= -1), $tipoPagamento]);
		}
	...
	//foreach($valores as $valor){
		//echo $valor[0]. '<br><br>';
	//}
	$url = "http://127.0.0.1/gerenciamentoapm/gestaoProdutos/uniformes.php";
	...
	/*
	FUNÇÃO CALCULO E REGISTRO NO BD
	$indice2 = 0;
	$novoValor = 0;
	while($indice2 < ($i-1)){
		if(isset($_POST[$uniformes[$indice2][0]])){
